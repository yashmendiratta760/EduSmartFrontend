...
@Composable
fun Signup(loginSignupViewModel: LoginSignupViewModel,
           ...
           loginUiState: LoginUiState)
{
    ...
    var userType = remember { mutableStateOf("") }
    var expanded by remember { mutableStateOf(false) }
    val userTypesAvailable = listOf<String>("TEACHER","STUDENT","PARENTS")
    ...
    LaunchedEffect(loginUiState.isLoggedIn) {
        if (loginUiState.isLoggedIn){
            TokenManager.getUserType(context).collect{user ->
                ...
                navController.navigate(if(user=="STUDENT") Screens.Student.name else Screens.Teacher.name ) {
                    popUpTo(0) {
                        inclusive = true
                    }
                }
    ...
    LaunchedEffect(loginUiState.canVerify) {
        if (loginUiState.canVerify){
            navController.navigate(Screens.OtpVerify.name)
        }
    }
...
//                    color = Color.Black)
            ...
            Button(
                ...
                modifier = Modifier
                    ...
                    loginSignupViewModel.signup(UserDTO(email, pass, userType = userType.value))
